...
- 우표 종류는 코드가 아니라 `stamps` 테이블의 행으로 관리한다. 한글 이름 · 한 줄 설명 · **이미지 파일 키**를 DB 에 저장하고, 추가·교체는 행 삽입/수정으로 끝낸다 ([ERD §2.4](./ERD.md#24-stamps--우표-마스터)). **우표 이름을 Java enum 으로 정의하지 않는다.**
