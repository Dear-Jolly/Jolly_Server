...
- DB 마이그레이션: **Flyway 필수**. 전 환경 `ddl-auto: validate` 로 고정한다. ERD 부록의 DDL 이 `V1__init.sql` 의 내용이다.
- **마이그레이션과 엔티티의 일치는 CI 가 검증한다.** 일반 테스트는 H2 + `ddl-auto: create-drop` 이라 엔티티로 스키마를 새로 만들고 `V1__init.sql` 을 한 번도 실행하지 않는다. 그 상태로는 둘이 어긋나도 전부 초록불이 뜨고, 운영의 `validate` 에서 **기동 실패**로 처음 드러난다. `SchemaMigrationTest` 가 Testcontainers 로 실제 MySQL 8 을 띄워 Flyway → `validate` 경로를 그대로 재현한다. 이 테스트는 Docker 를 요구한다 (Colima 사용 시 `DOCKER_HOST` · `DOCKER_API_VERSION` 을 내보내야 한다).
...
| 마이그레이션 검증 | Testcontainers 로 실제 MySQL 에서 `validate` | 나머지 테스트는 H2 + `create-drop` 이라 마이그레이션을 실행조차 하지 않는다. 검증 장치가 없으면 "Flyway 가 정본" 이라는 규칙이 운영 기동 실패로만 확인된다 |
