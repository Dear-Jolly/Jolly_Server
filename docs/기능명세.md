...
- 최소 지원 버전 미만이면 앱이 강제 업데이트를 유도한다. **판정은 앱이 한다.** 서버는 클라이언트 버전을 받지 않으므로 개별 요청이 업데이트 대상인지 알 수 없고, 응답의 `forceUpdate` 는 `latestVersion == minSupportedVersion` 일 때 켜지는 보조 신호일 뿐이다.
- 응답 값은 전부 서버 설정(`dearjolly.version.*`)에서 온다. `platform`(`IOS` · `AOS`)을 주면 그 플랫폼에 재정의된 값만 공통 값을 덮어쓴다 — 한쪽만 심사에 걸려 버전이 벌어지는 상황을 흡수하기 위한 것이다.
...
| 키 | 기본값 | 설명 |
| --- | --- | --- |
| `APP_LATEST_VERSION` / `APP_MIN_SUPPORTED_VERSION` | `1.0.0` | 공통 값 |
| `APP_LATEST_VERSION_IOS` / `APP_MIN_SUPPORTED_VERSION_IOS` | 없음 | 채우면 iOS 만 공통 값을 덮어쓴다 |
| `APP_LATEST_VERSION_AOS` / `APP_MIN_SUPPORTED_VERSION_AOS` | 없음 | 채우면 Android 만 공통 값을 덮어쓴다 |
