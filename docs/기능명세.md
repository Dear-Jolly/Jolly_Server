...
>
> **구현 현황** — 인증 · 온보딩 · 계정(§3.1~§3.2)과 버전 조회(§3.7)는 구현과 통합 테스트를 마쳤다.
> 편지 · 홈 · AI 피드백 파이프라인(§3.3~§3.6)과 Rate limit(§4.2)은 명세만 확정된 상태다.
...
**`state`**: authorize URL 에 UUID 를 실어 보내고 콜백으로 그대로 돌려받지만 **MVP 는 검증하지 않는다** (§6).
...
- 우표 후보는 매 호출 시 `SELECT name FROM stamps` 로 조회해 **프롬프트에 동적으로 삽입**한다. 필터 조건이 없다 — 테이블의 모든 행이 후보다. 우표가 늘거나 줄어도 프롬프트 템플릿과 코드는 그대로다 (§3.5.5).
...
implementation 'org.springframework.boot:spring-boot-starter-actuator'   // 헬스체크 (§4.3)
...
testImplementation 'org.springframework.boot:spring-boot-testcontainers'  // 마이그레이션 검증 (§4.3)
...
```gradle
implementation 'io.github.java-diff-utils:java-diff-utils:4.12'  // 교정 세그먼트 diff (§3.5.3)
```
...
| `JPA_DDL_AUTO` | `validate` | 전 환경 고정. 스키마는 Flyway 가 소유한다 (§4.3) |
| `FLYWAY_ENABLED` | `true` | 마이그레이션 실행 여부. 기본값을 유지한다 (§4.3) |
...
| 키 | 설명 |
| --- | --- |
| `OPENAI_API_KEY` / `OPENAI_MODEL` | OpenAI 키와 모델 ID (§3.5.2) |
...
| 인증 필터 제외 | 필터가 경로를 직접 보고 건너뛴다 | `permitAll` 은 인가만 열 뿐 필터를 막지 못한다. 재발급은 만료된 토큰을 들고 오는 것이 정상이라, 구분하지 않으면 재발급 자체가 `AUTH_005` 로 막힌다 (§3.1.2) |
...
| 마이그레이션 검증 | Testcontainers 로 실제 MySQL 에서 `validate` | 나머지 테스트는 H2 + `create-drop` 이라 마이그레이션을 실행조차 하지 않는다. 검증 장치가 없으면 "Flyway 가 정본" 이라는 규칙이 운영 기동 실패로만 확인된다 (§4.3) |
| 닉네임 검증 위치 | 요청 DTO 가 아니라 서비스 | Bean Validation 을 태우면 사유가 `COMMON_001` 하나로 뭉개진다. 앱이 사유별 문구를 띄워야 하므로 코드가 사유를 구분해야 한다 (§3.2.2) |
