...
- **`GET /api/v1/home` 은 `letter` 도메인**에 둔다. 닉네임은 유저에서 오지만 응답의 본질은 편지 집계(우표 수)이고, `GET /api/v1/letters` 와 같은 서비스 메서드를 공유해야 하기 때문이다.
...
- 우표 수 조회는 홈 정보 조회와 동일한 메서드를 공유하도록 서비스 레이어에서 한 곳으로 모은다.
