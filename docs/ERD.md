...
erDiagram
    ...
    LETTERS {
        ...
        DATETIME next_retry_at "다음 재시도 예약 시각"
...
| `stamp_id` | BIGINT | FK → `STAMPS.stamp_id`, NULL | 편지에 붙은 우표. 등록 시 `soon`, 첫 실패 시 `fail`, `FEEDBACK_COMPLETED` 전이 시 LLM 이 고른 우표가 된다 |
| `is_read` | BOOLEAN | NOT NULL, DEFAULT `false` | 피드백 열람 여부 (R6). `FEEDBACK_COMPLETED` 편지를 상세 조회할 때만 `true` 로 전환 |
| `retry_count` | INT | NOT NULL, DEFAULT `0` | OpenAI 재시도 횟수. 최초 호출은 제외하고 최대 5회 |
| `next_retry_at` | DATETIME(6) | NULL | DB에 영속화한 다음 재시도 예약 시각 |
...
- 편지는 append-only 다 (R1). 등록 후 본문은 변경하지 않으며, 처리 과정에서 `status`, `stamp_id`, `is_read`, `retry_count`, `next_retry_at`, `version`, `updated_at`이 변경된다.
...
- **활성 여부 컬럼을 두지 않는다.** 후보에서 빠지는 행은 서버가 상태에 따라 직접 붙이는 `soon` · `fail` 둘뿐이고, 이름으로 걸러진다. "있지만 안 쓰는 우표" 라는 상태는 실체가 없어 플래그 컬럼을 둘 이유가 없다.
...
`fail` 은 첫 실패 시점에 붙는 "실패" 우표다. 같은 폴더의 `fail.png` 로 시드되며 역시 후보에서 빠진다.
...
1. 피드백 워커가 `SELECT name FROM stamps WHERE name NOT IN ('soon', 'fail')` 으로 후보를 조회한다. 서버가 직접 붙이는 두 우표를 제외한 모든 행이 후보다.
...
| `FEEDBACK_FAILED` | 한 번 이상 실패 | 서버가 내려준 `fail` 우표, 실패 안내, 상세 진입 불가 |

**컬럼 값과 응답 값은 다르다.** `LETTERS.status` 는 내부 상태를 그대로 담고, 앱 응답의 `status` 는 여기에 `retry_count` 를 얹어 계산한다.

| 컬럼 `status` | `retry_count` | 응답 `status` |
| --- | :---: | --- |
| `SUBMITTED` · `FEEDBACK_IN_PROGRESS` | `0` | 컬럼 값 그대로 |
| `SUBMITTED` · `FEEDBACK_IN_PROGRESS` | `>= 1` | `FEEDBACK_FAILED` |
...
| INDEX | `LETTERS (user_id, letter_date DESC, letter_id DESC)`, `LETTERS (status, next_retry_at)`, `LETTERS (status, updated_at)`, `TERMS_AGREEMENTS (user_id, type, agreed_at DESC)` |
| DEFAULT | `LETTERS.is_read = false`, `LETTERS.retry_count = 0`, `LETTERS.version = 0` |
...
| A18 | `retry_count >= 1` 이고 완료 전이면 응답 `status` 는 `FEEDBACK_FAILED` 로 내린다. 내부 재시도는 앱에 노출하지 않는다 | 실패 안내 번복 방지 | `Letters.toResponseStatus()` |
...
편지는 append-only 다. 작성되면 `SUBMITTED`, 워커가 픽업하면 `FEEDBACK_IN_PROGRESS`, 저장에 성공하면 `FEEDBACK_COMPLETED`로 전이한다. 5회의 단일 재시도 예산을 소진하면 `FEEDBACK_FAILED`로 확정한다.
첫 실패 시점에 `stamp_id` 가 `fail` 로 바뀌고, 그때부터 앱 응답 상태는 `FEEDBACK_FAILED` 로 고정된다 (A18).

| 전이 | 트리거 | 함께 갱신되는 컬럼 |
| --- | --- | --- |
| → `SUBMITTED` | 편지 작성 | `stamp_id` 에 기본 우표(`soon`) 부여, `is_read = false`, `retry_count = 0`, `next_retry_at = now`, `version = 0` |
...
CREATE TABLE letters (
    ...
    KEY idx_letters_retry (status, next_retry_at),
    KEY idx_letters_stalled (status, updated_at)
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4;
