...
앱 최소 지원 버전은 환경변수가 아니다. `APP_VERSIONS` 테이블에 있고 `PATCH /api/v1/admin/version` 으로만 바꾼다.
...
OpenAI 호출 실패는 30초, 2분, 5분, 30분, 1시간 간격으로 최대 5회 재시도한다.
실패할 때마다 `retry_count`와 `next_retry_at`을 DB에 저장하고 평상시에는 인메모리 스케줄러가 재실행한다.
복구 배치는 애플리케이션 기동 직후와 매일 0시(KST)에 돌며, 유실된 예약과 15분 이상 멈춘 작업을 DB에서 찾아
같은 재시도 예산으로 복구한다. 재시도 예약이 인메모리라 배포·재기동으로 사라지기 때문에 기동 직후 스캔이 필요하다.
주기를 임시로 바꿔야 하면 `.env` 에 `DEARJOLLY_FEEDBACK_RECOVERYCRON` 을 넣는다 (기본 `0 0 0 * * *`).
첫 실패 시점에 편지 우표를 `fail` 로 바꾸고 앱 응답 상태를 `FEEDBACK_FAILED` 로 고정한다. 내부 재시도가 성공하면
편지에 맞는 우표와 `FEEDBACK_COMPLETED` 로 한 번에 갱신된다. 5차 재시도도 실패하면 `FEEDBACK_FAILED` 로 확정되며
이후에는 관리자가 명시적으로 재처리해야 한다.
...
| 최소 지원 버전을 올리고 싶다 | `PATCH /api/v1/admin/version` 을 호출한다. 배포도 재기동도 필요 없다 |
...
`soon.png` 는 편지 등록 시점에 붙는 "준비 중" 우표이고, `fail.png` 는 첫 실패 시점에 붙는 "실패" 우표다.
둘 다 서버가 상태에 따라 직접 붙이므로 LLM 우표 선택 후보에서는 빠진다. `soon` 은 `stamp_id = 1` 로 고정된다.
두 파일 모두 `seed/stamps/` 에 있으므로 별도 업로드 없이 시드가 MinIO 에 함께 올린다.
