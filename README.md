...
| 🤖 **LLM API** | 편지 피드백은 **비동기**다. 작성 응답은 즉시 돌아가고, 답장이 완료된 뒤 편지에 맞는 우표가 부여된다. 생성이 한 번이라도 실패하면 실패 우표로 바뀌고, 서버가 재시도해 성공하면 다시 갱신된다 |
...
| [docs/AI로직.md](docs/AI로직.md) | 피드백 생성·재시도·복구 동작 |
| [docs/로그체계.md](docs/로그체계.md) | 비즈니스 로그 이벤트와 장애 추적 |
| [docs/프론트변경사항.md](docs/프론트변경사항.md) | 앱에 영향을 주는 최근 변경 |
